...
tags:
  ...
  - Markdown
  - Cloudflare
description: 从 AstroPaper 开始，完成中文适配、文章与专题、静态搜索和发布配置，把搭建博客的过程变成第一篇学习笔记。
---

我想要一个地方，记录平时学到的知识：一次排查问题的过程、一段值得保留的代码，以及对某个技术选择的理解。于是有了「折腾花园」。

这篇文章记录本站的第一版实现，也整理了连接 GitHub、部署到 Cloudflare Pages 和启用统计的后续操作。**本地完成与线上开通是两个步骤**：前者可以通过代码和构建验证，后者需要在自己的账号下创建项目、配置域名并确认部署结果。
...
本节是部署操作说明。创建仓库、托管项目和绑定域名，需要使用自己的账号完成；配置文件本身不能代表已经上线。
...
| Node.js    | `24`                            |
| pnpm       | `11.19.0`                       |
| `SITE_URL` | 实际的正式网址，包含 `https://` |

不同构建镜像的包管理器检测方式可能不同；以构建日志为准，确认使用了仓库声明的版本。本站的 GitHub Actions 也会按 `.nvmrc` 和 `packageManager` 安装环境并运行构建，便于及早发现错误。

Pages 会在推送新提交后构建并发布。首次可以使用平台分配的 `pages.dev` 地址，确认页面正常后，再在项目的 Custom domains 中添加自己的域名，按提示完成 DNS 配置。
