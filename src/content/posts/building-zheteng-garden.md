...
第一次推送后，GitHub 对上游锁文件里的依赖给出了安全告警。这提醒我：锁文件有助于复现环境，也需要持续维护。

本站移除了已不再使用的分享图依赖 `satori`，在现有主版本范围内更新依赖，将 Astro 升到 7.3.2、Sharp 升到 0.35.4。更新后仍有一条可选依赖链固定了旧版内部工具包，因而在 `pnpm-workspace.yaml` 中将 `@astrojs/internal-helpers@0.10.0` 定向覆盖为同一兼容范围内的 0.10.4，以使用修复后的 YAML 和 TOML 解析器。以后上游不再依赖旧版本时，可以移除这条覆盖。

依赖更新后要重新运行 `pnpm audit` 和完整构建，再检查搜索、文章渲染等实际功能。此次修复后的 `pnpm audit` 未报告已知漏洞。本站是静态部署，云端提供的是生成的 HTML、CSS 和 JavaScript；依赖扫描发现的问题仍应按具体使用方式判断，并及时修复。
